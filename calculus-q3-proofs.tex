...
\item Use Lagrange Multipliers to prove that the triangle with maximum area that
  ...
  \textit{Hint}: Use the Heron's formula for the area:
...
\item If \(f(x,y)\) is continuous on \([a,b] \times [c,d]\) and
  \(g(x,y) = \int_0^x \int_0^y f(s,t) \, dt \, ds\) for
...
\item
  ...
  \,dx \,dy = \int_0^4 \int_{\frac{2}{3}(x-1)}^{\sqrt{x}} x^2 \sin(x-y) \,dy
  \,dx\)

\item
  \(\int_0^{\pi/4} \int_{\sin x}^{\cos x} \sqrt{x + y^2} \, dy \, dx = \int_0^1
  \int_{\arcsin y}^{\arccos y} \sqrt{x + y^2} \, dx \, dy\)

\item \(\int_{-1}^{1} \int_{0}^{1} e^{x^2 + y^2} \sin y \, dx \, dy = 0\)
\end{enumerate}
